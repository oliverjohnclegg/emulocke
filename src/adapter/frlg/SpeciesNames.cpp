...
constexpr const char* kHoenn[] = {
    ...
    "MEDICHAM",  "SWABLU",     "ALTARIA",   "WYNAUT",    "DUSKULL",   "DUSCLOPS",  "ROSELIA",   "SLAKOTH",
    "VIGOROTH",  "SLAKING",    "GULPIN",    "SWALOT",    "TROPIUS",   "WHISMUR",   "LOUDRED",   "EXPLOUD",
    ...
    "SALAMENCE", "BELDUM",     "METANG",    "METAGROSS", "REGIROCK",  "REGICE",    "REGISTEEL", "KYOGRE",
    "GROUDON",   "RAYQUAZA",   "LATIAS",    "LATIOS",    "JIRACHI",   "DEOXYS",    "CHIMECHO",
};
...
const char* frlgSpeciesName(uint16_t species) {
    ...
    if (species >= 277) {
        const uint16_t i = static_cast<uint16_t>(species - 277);
        if (i < sizeof(kHoenn) / sizeof(kHoenn[0])) {
            return kHoenn[i];
        }
    }
    return "???";
