...
Pokemon fromSpecies(uint16_t species, uint8_t level, uint8_t iv, uint8_t nature, uint8_t abilityNum,
    const uint8_t ev[6]) {
    Pokemon out;
    const SpeciesRow* row = speciesById(species);
    if (!row) {
        return out;
    }
    out.name = row->slug;
...
Pokemon pokemonFromPack(const PackMon& mon) {
    ...
    const uint8_t ev[6]{};
    Pokemon out = fromSpecies(mon.species, mon.level, iv, 0, 0, ev);
    out.item = mon.item;
    return out;
}
...
Pokemon pokemonFromSnap(const Mon& mon) {
    ...
    const uint8_t iv = mon.ivAtk;
    Pokemon out = fromSpecies(mon.species, mon.level, iv, mon.nature, mon.abilityNum, ev);
    if (mon.maxHp) {
        out.maxHp = mon.maxHp;
        out.hp = mon.hp;
        out.atk = mon.attack;
...
void applyBattler(Pokemon& mon, const BattleBattler& b) {
    ...
    if (b.hp) {
        mon.hp = b.hp;
        mon.maxHp = b.maxHp ? b.maxHp : mon.maxHp;
    }
    mon.status = b.status;
}
