...
void drawTitleStrip(Application& app, const CatalogTitle& title, ImVec2 min, ImVec2 max, bool ready) {
    ...
    if (display) {
        dl->AddText(display, nameSize, text, nameCol, title.title);
    } else {
        dl->AddText(text, nameCol, title.title);
    }
...
bool drawTitleStripRow(Application& app, const CatalogTitle& title, std::string& catalogUuid) {
    ...
    if (!ready && ImGui::IsItemHovered()) {
        ImGui::SetTooltip("%s", title.kind == TitleKind::Hack
                                    ? "The dependency game needs to be imported first."
                                    : "This game needs to be imported to play.");
    }
