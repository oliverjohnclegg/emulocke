...
- **Third-party undefined behaviour.** xdelta3 performs intentional unaligned loads in its checksum and uses the `&((T*)0)->member` offsetof idiom. The xdelta3 target compiles with `-fno-sanitize=alignment,null,object-size,pointer-overflow` under `EMULOCKE_SANITIZE=address`; everything else in that library stays instrumented. mGBA and melonDS are instrumented in full.
...
`.cursor/rules/cpp-memory-safety.mdc` applies to every C++ change and points at the vendored skills under `.cursor/skills/` (`cpp-coding-standards`, `cpp-sanitizers`, `memory-safety-patterns`). Parser changes ship an adversarial test and a fuzz seed; nothing is done until the sanitizer suite and `clang-tidy` are clean.
