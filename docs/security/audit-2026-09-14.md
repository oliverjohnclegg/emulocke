...
- **Third-party undefined behaviour.** xdelta3 has several UB idioms (unaligned loads, null-base offsetof, memcpy from a null `next_out`, secondary-compressor function-pointer types). ASan stays on for that target; UBSan does not, because `-fno-sanitize-recover=all` would abort the fuzzer on those library bugs. mGBA and melonDS stay fully instrumented.
- **xdelta fail-closed.** `applyXdeltaPatch` still rejects a target-window checksum mismatch. Bundled hacks go through `applyRomPatch`, which retries once with `XD3_ADLER32_NOVER` so Drayano's Blaze Black / Volt White xdelta files apply. Hostile input in `fuzz_xdelta` still hits the strict decoder.
