...
- Emulator lab tools stay out: save states, rewind, disassemblers, memory viewers, movie recording, Lua, scanline filters, HUD counters. The suite Cheats tab is the exception: pasteable Action Replay / GameShark / Codebreaker codes, not a searcher.
...
- Right column is the suite. Tabs hold each function. Tracker is first. Pokémon is second. Calculator is third. Cheats is fourth. Logs is fifth. A catalog title is represented only when live party and PC, the tracker atlas (with auto-capture and auto-boss), and a calc pack all exist. Incomplete titles show the empty plate, not a half-working tracker. Difficulty is derived from the cart when the hack has an in-game setting. Pokémon lists the live party (with HP), boxed mons as box sprites, and a Grave of tracker-dead mons. Logs shows trainer, map, party, and badges for supported games. Calculator searches trainers and locations, then reads damage into the current party. Cartridge generation is not damage generation. CFRU titles do not inherit FireRed gen 3 math. Cheats are user-pasted Action Replay / GameShark / Codebreaker codes, stored per catalog title in the SDL pref path. Enable toggles live on the run. New runs and new attempts start with every cheat off. New Run Nuzlocke Settings has Allow Cheats? (default on); off hides the Cheats tab and does not change Regular or Hardcore. No bundled lists.
- With no run seated, the left column is home. Empty home centers a START RUN hero. When plates exist, START RUN is a compact full-width stamp rail (plus + name) in the same graphite chrome as the plates. Each plate is `GAME - Preset`, a subtitle of host playtime as `HH:MM` • Attempt #N • Deaths • Badges, title art, and six party sockets. NEW ATTEMPT is a plus icon with a hover name. Click a plate to load. Cart status never sits above START RUN or in New Run; it stays in Logs. The right column shows only Logs until a run is seated. Tracker, Pokémon, Calculator, and Cheats return with the run. Tracker is empty until a represented title is seated.
...
- Pause, reset, and speed-up (Tab, default 3x). Esc pauses. Emulation > Speed-up holds the 2x-8x slider and Hold Tab vs toggle.
...
- Tracker is a compact location/boss list. A seated title is represented only when its bundle (live snapshot, atlas, calc pack) exists. Difficulty is derived from the seated cart and persisted as `difficulty=` on `meta.ini`. Encounter state lives in `tracker.ini` on the run.
...
- Emulator lab tools (save states, rewind, disassemblers, memory viewers, movies, Lua, filters, HUD). The Cheats tab is suite QoL, not a searcher.
...
- Adapters stay read-only. RAM writes go through the Cheats tab and the core cheat engines, not `GameAdapter`.
...
- A title is represented only when the adapter fills party, PC, map, gyms, flags, starter, `species()`, and trainer battle overlay; the atlas has ordered stops with mets or starter on every encounter and `defeat` or gym index on every boss; and `calcPack(uuid, difficulty)` is non-null. Incomplete titles show the empty plate on Tracker, Pokémon, and Calculator. Version twins share one bundle. In-game difficulty picks a pack/atlas variant where teams diverge (Radical Red Hardcore, Unbound Expert). Missed stays a manual click.
