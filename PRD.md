...
- View > Screen Scale resizes the whole window so the screens fill the left pane at that integer size. Fit keeps the current window and integer-scales inside it. Default window fits 2x two-screen plus a 460px suite.
...
- 2026-09-13: View > Screen Scale snaps the window to the LCD cluster so the screens fill the left pane. Fit keeps the current window.
